...
def design_initial_filter(numtaps, bands, desired, weights):
    ...
    print(f"Time taken to calculate initial filter coefficients of non-digitized adaptive filter: {elapsed_time:.4f} seconds")
...
def adaptive_filter_lms(initial_filter, input_signal, desired_signal, mu, num_iterations):
    ...
    elapsed_time = time.time() - start_time
    print(f"Time taken to calculate final filter coefficients of non-digitized adaptive filter: {elapsed_time:.4f} seconds")
