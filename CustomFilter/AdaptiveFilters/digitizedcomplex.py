...
# Step 4: Define Function for Adaptive Filtering (LMS Algorithm) with Complex Coefficients
def adaptive_filter_lms_complex(initial_filter, input_signal, desired_signal, mu, num_iterations):
    filter_coeffs = initial_filter.copy().astype(complex)
    numtaps = len(initial_filter)
    output_signal = np.zeros(num_iterations, dtype=complex)
    error_signal = np.zeros(num_iterations, dtype=complex)

    for n in range(numtaps, num_iterations):
        x = input_signal[n - numtaps:n]
        y = np.dot(filter_coeffs, x)
        e = desired_signal[n] - y
        filter_coeffs += 2 * mu * np.conj(e) * x
        output_signal[n] = y
        error_signal[n] = e

    return output_signal, error_signal, filter_coeffs

# Step 5: Define Function to Generate Signals with Complex Noise
def generate_signals_complex(num_iterations, input_freq, noise_level, sampling_freq):
    t = np.arange(num_iterations) / sampling_freq
    desired_signal = np.exp(1j * 2 * np.pi * input_freq * t)  # Complex sinusoid
    noise = noise_level * (np.random.randn(num_iterations) + 1j * np.random.randn(num_iterations))
...
def decimate_coefficients(coefficients):
    ...
    decimated_coeffs = np.where(np.abs(coefficients) >= dynamic_threshold, 1 + 1j, 0 + 0j)
    return decimated_coeffs

# Step 9: Binary LMS Algorithm with Complex Coefficients
def binary_lms_complex(initial_filter, input_signal, desired_signal, mu, num_iterations):
    filter_coeffs = initial_filter.copy().astype(complex)
    numtaps = len(initial_filter)
    output_signal = np.zeros(num_iterations, dtype=complex)
    error_signal = np.zeros(num_iterations, dtype=complex)
    ...
    for n in range(numtaps, num_iterations):
        ...
        y = np.dot(filter_coeffs, x)
        e = desired_signal[n] - y
        filter_coeffs = filter_coeffs + 2 * mu * np.conj(e) * x
        filter_coeffs = np.where(np.abs(filter_coeffs) >= np.median(np.abs(filter_coeffs)), 1 + 1j, 0 + 0j)
        output_signal[n] = y
        error_signal[n] = e
...
def run_filter(numtaps, bands, desired, weights, mu, num_iterations, input_freq, noise_level, sampling_freq):
    ...
    input_signal, desired_signal = generate_signals_complex(num_iterations, input_freq, noise_level, sampling_freq)
    ...
    output_signal, error_signal, final_coeffs = binary_lms_complex(decimated_initial_filter, input_signal, desired_signal, mu, num_iterations)
    ...
    plt.subplot(4, 2, 1)
    plt.plot(input_signal.real, label='Input Signal (Real Part)')
    ...
    plt.subplot(4, 2, 2)
    plt.plot(input_signal.imag, label='Input Signal (Imaginary Part)')
    ...
    plt.subplot(4, 2, 3)
    plt.plot(desired_signal.real, label='Desired Signal (Real Part)')
    plt.title('Desired Signal (Real Part)')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    plt.legend()

    plt.subplot(4, 2, 4)
    plt.plot(desired_signal.imag, label='Desired Signal (Imaginary Part)')
    plt.title('Desired Signal (Imaginary Part)')
    plt.xlabel('Sample Index')
    plt.ylabel('Amplitude')
    plt.legend()

    plt.subplot(4, 2, 5)
    plt.plot(output_signal.real, label='Output Signal (Real Part)')
    ...
    plt.subplot(4, 2, 6)
    plt.plot(output_signal.imag, label='Output Signal (Imaginary Part)')
    ...
    plt.subplot(4, 2, 7)
    plt.plot(np.abs(np.fft.fft(output_signal)), label='Output Signal (Magnitude)')
    plt.plot(np.abs(np.fft.fft(desired_signal)), label='Desired Signal (Magnitude)')
    plt.plot(np.abs(np.fft.fft(input_signal)), label='Input Signal (Magnitude)')
    plt.title('Output, Desired, and Input Signals (Magnitude)')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Magnitude')
    plt.legend()

    plt.subplot(4, 2, 8)
    plt.plot(np.angle(output_signal), label='Output Signal (Phase)')
    plt.plot(np.angle(desired_signal), label='Desired Signal (Phase)')
    plt.plot(np.angle(input_signal), label='Input Signal (Phase)')
    plt.title('Output, Desired, and Input Signals (Phase)')
    plt.xlabel('Frequency (Hz)')
