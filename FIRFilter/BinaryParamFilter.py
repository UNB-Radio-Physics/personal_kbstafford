...
def digitize_coefficients(coefficients, threshold=0.5):
    #Digitize the filter coefficients to 0 or 1 based on a threshold.
    return [1 if coeff >= threshold else 0 for coeff in coefficients]

def display_coefficients(b, a):
    #Display the digitized coefficients in the text box.
    text_box.delete('1.0', END)  # Clear previous content
    text_box.insert(END, "Numerator Coefficients:\n")
    for coeff in b:
        text_box.insert(END, f"{coeff}\n")
...
def apply_filter():
    try:
        ...
        threshold = 0.5  # Threshold for digitizing coefficients to 0 or 1
        ...
        b_digital = digitize_coefficients(b, threshold)
        a_digital = digitize_coefficients(a, threshold)

        # Display coefficients in the text box
        display_coefficients(b_digital, a_digital)
...
notebook.add(tab2, text='Coefficients')
...
# Text box to display coefficients
text_box = Text(tab2, wrap='word', height=30, width=80)
